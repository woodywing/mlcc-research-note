...
このリポジトリは調査継続用の作業資料なので、MLCC に関する整理・出典確認・仮説管理のための更新は積極的に行ってください。過度に慎重になって都度確認するより、`main` や作業ブランチ上で調査メモを育てることを優先します。

- `main` は作業資料としての基準ブランチです。調査メモや途中整理が散らばっていてもよく、必要な編集、コミット、プッシュは原則として都度承認を求めずに進めて構いません。
- `formal` は正式利用・共有向けの安定ブランチです。このブランチだけは直接更新を避け、別ブランチから Pull Request を出してレビュー・マージする方式で更新してください。
- `formal` 以外のブランチ作成、派生ブランチでの編集、コミット、プッシュは、原則として都度承認を求めずに進めて構いません。
- ブランチはむやみに増やさず、タスク単位またはトピック単位で意味が分かる名前にしてください。この「必要に応じてブランチを切る」運用思想は維持してください。
